...
switch($desg){
    ...
    case "Principal" : $x="HOD";
        break;
    case "HOD" : $x="Professor";
        break;
    case "Professor" : $x = "Professor";
        break;
}
        ...
        <div class="container bg-inverse text-white">
          <h1>Welcome <?php echo $name; ?> </h1> 
            ...
            <div class="col-sm-9 bg-faded text-muted">
                <br />            
                 <div class="title"> Select Verifiers for the new document to all <?php echo $x;?>:</div>
                    ...
                    <tbody>
                    <?php 
                        $validate = "SELECT * FROM faculty where name = '".$name."' or designation = '".$x."'";
                        ...
                        foreach ($result2 as $row)
                        {
                            echo "<tr>
                                    ...
                                    <td> ". $row['dept']."</td>
                                    <td> ";
                            $validate = "SELECT * FROM verify WHERE fname = '".$row['name']."' AND title = '".$title."' ";
                            $result2 = mysqli_query($con,$validate);
                            if(mysqli_num_rows($result2) > 0)
                                    { echo "Verifier Set. </td> </tr>";}
                            else 
                            {
                             //code   
                            echo "<a href=\"verifycode2.php?name=".$row['name']."&title=".$title." \"> Click here to Add  </a> </td> </tr>";
                            }
                             }    
                    ?>    
                    </tbody>
