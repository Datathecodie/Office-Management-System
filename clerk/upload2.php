...
// Allow certain file formats
    ...
    if (move_uploaded_file($_FILES["filepath"]["tmp_name"], $target_file) ) {
        ...
        $insert ="Insert into document (title,description,dept,path,date,uname,forc) value ('".$title."','".$desc."','".$dept."','".$target_file."','".$date1."','".$name."','clerk')";
    $result = mysqli_query($con,$insert);
        $insert ="Insert into request (title,done,date) value ('".$title."','no','".$date1."')";
        $result = mysqli_query($con,$insert);
        $_SESSION['title']=$title;
    header("Location:upload.php?error=Upload Succesful ");
